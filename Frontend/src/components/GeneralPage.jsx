  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        ...
        const response = await api.get("/api/items"); // ✅ with token
        const responseData = response?.data?.data || [];

        console.log("Categories found:", [...new Set(responseData.map(i => i.category))]);

        // ✅ Fix category filter to match backend ("General", not "Generals")
        const generals = responseData.filter((item) =>
          item.category?.trim().toLowerCase() === "general"
        );
